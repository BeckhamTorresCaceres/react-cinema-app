...
const DriftWall = ({
  ...
  const captionClass = cx(
    'pointer-events-none absolute inset-x-0 bottom-0 z-10 opacity-0',
    'transition-opacity duration-[420ms] ease-[cubic-bezier(0.22,1,0.36,1)]',
      ...
      <span className={innerClass}>
        ...
        {item.title !== undefined || item.description !== undefined ? (
          <span className={captionClass}>
            <span
              className="pointer-events-none absolute inset-0 bg-linear-to-t from-[#080616] via-[#080616]/60 to-transparent"
              aria-hidden="true"
            />
            <span className="relative block p-3">
              ...
              {item.description && (
                <span className="mt-1 line-clamp-2 block text-xs leading-snug text-slate-300">
                  {item.description}
                </span>
              )}
            </span>
